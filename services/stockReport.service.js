...
function getTemplatePath(type) {
  if (type === "Incoming") {
    return path.join(
      process.cwd(),
      "templates",
      "stock-report-incoming.mustache",
    );
  }
  if (type === "Outgoing") {
    return path.join(
      process.cwd(),
      "templates",
      "stock-report-outgoing.mustache",
    );
  }
  throw new Error(
    `Unknown warehouseMode: "${type}". Expected "Incoming" or "Outgoing".`,
  );
}
...
function getFooterHtml(viewModel) {
  if (viewModel.warehouseMode === "Incoming") {
    return `
      <div style="display:flex; gap:8px; margin-bottom:10px; font-family:Arial,Helvetica,sans-serif;">
        <div style="flex:1 1 50%; display:flex; align-items:center; white-space:nowrap;">
          <div style="width:50%; font-size:12px;">Encoded By:</div>
          <div style="width:50%; font-size:12px; padding-left:6px;">${escapeHtml(viewModel.encodedBy || "")}</div>
        </div>
        <div style="flex:1 1 50%; display:flex; align-items:center; white-space:nowrap;">
          <div style="width:50%; font-size:12px;">Encoded Date:</div>
          <div style="width:50%; font-size:12px; padding-left:6px;">${escapeHtml(viewModel.encodedDate || "")}</div>
        </div>
      </div>
      <div style="display:flex; gap:8px; margin-bottom:30px;">
        <div style="flex:1 1 50%; display:flex; align-items:center; white-space:nowrap;">
          <div style="width:50%; font-size:12px;">Verified By:</div>
          <div style="width:50%; font-size:12px; padding-left:6px;">${escapeHtml(viewModel.verifiedBy || "")}</div>
        </div>
        <div style="flex:1 1 50%; display:flex; align-items:center; white-space:nowrap;">
          <div style="width:50%; font-size:12px;">Verified Date:</div>
          <div style="width:50%; font-size:12px; padding-left:6px;">${escapeHtml(viewModel.verifiedDate || "")}</div>
        </div>
      </div>
    `;
  ...
  if (viewModel.warehouseMode === "Outgoing") {
    return `
      <div style="display:flex; gap:8px; margin-bottom:10px; font-family:Arial,Helvetica,sans-serif;">
        <div style="flex:1 1 50%; display:flex; align-items:center; white-space:nowrap;">
          <div style="width:50%; font-size:12px;">Released By:</div>
          <div style="width:50%; font-size:12px; padding-left:6px;">${escapeHtml(viewModel.releasedBy || "")}</div>
        </div>
        <div style="flex:1 1 50%; display:flex; align-items:center; white-space:nowrap;">
          <div style="width:50%; font-size:12px;">Received By:</div>
          <div style="width:50%; font-size:12px; padding-left:6px;">${escapeHtml(viewModel.receivedBy || "")}</div>
        </div>
      </div>
      <div style="width:50%; display:flex; gap:8px; margin-bottom:30px;">
        <div style="flex:1 1 50%; display:flex; align-items:center; white-space:nowrap;">
          <div style="width:50%; font-size:12px;">Date:</div>
          <div style="width:50%; font-size:12px; padding-left:6px;">${escapeHtml(viewModel.date || "")}</div>
        </div>
      </div>
    `;
...
async function renderSinglePdf(viewModel, browser) {
  ...
  try {
    ...
    const pdfBuffer = await page.pdf({
      ...
      margin: { top: "47mm", bottom: "40mm", left: "8mm", right: "8mm" },
    });
...
async function generateStockReportPdfBuffer(viewModelOrArray) {
